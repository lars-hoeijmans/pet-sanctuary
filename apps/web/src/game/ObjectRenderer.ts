/**
 * Renders StageObjects as real Kenney furniture sprites (CC0), sized to fit their
...
import { resolveObjectSprite } from "./assets";
import { TILE_W } from "./IsoMath";
import { TEX } from "./TextureFactory";

/** Normalized object types that show their label as a floating chip above the sprite. */
...
/** Normalize a free-form object type to the chip-lookup key. */
function normalizeType(type: string): string {
  return type.trim().toLowerCase().replace(/[\s-]+/g, "_");
}

export function createObjectView(scene: Phaser.Scene, object: StageObject): Phaser.GameObjects.Container {
  const spec = resolveObjectSprite(object.type);
  ...
  if (object.label && LABELLED_TYPES.has(normalizeType(object.type))) {
