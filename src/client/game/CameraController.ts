...
export class CameraController {
  ...
  constructor(scene: Phaser.Scene, cam: Phaser.Cameras.Scene2D.Camera, center: { x: number; y: number }) {
    this.cam = cam;
    this.center = center;
    ...
    scene.input.on(
      ...
      (_pointer: Phaser.Input.Pointer, _objects: unknown, _dx: number, dy: number) => {
        const next = Phaser.Math.Clamp(this.cam.zoom - dy * 0.0014, MIN_ZOOM, MAX_ZOOM);
  ...
  reset(): void {
    ...
    this.cam.zoomTo(1, 420, "Sine.easeInOut");
  }
}
