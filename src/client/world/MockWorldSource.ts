...
import { baseWorldObjects, randomTileInZone, zoneCenter } from "./zones";
...
export class MockWorldSource implements WorldSource {
  ...
  spawnTestAgent(): void {
    ...
    const pos = { ...zoneCenter("central-plaza"), x: 8 + (n % 5), y: 7 };
  ...
  buildRandomObject(): void {
    ...
    const pos = randomTileInZone(def.homeZoneId, this.rng);
  ...
  private freshWorld(): WorldSnapshot {
    ...
    for (const obj of baseWorldObjects()) world.objects[obj.id] = obj;
    return world;
  }
